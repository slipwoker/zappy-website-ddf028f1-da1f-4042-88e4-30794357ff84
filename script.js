document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const ERROR_CLASS = 'field-error-message';

    function getOrCreateErrorEl(field) {
      const wrapper = field.closest('.form-group, .field-wrapper, div') || field.parentNode;
      let errorEl = wrapper.querySelector('.' + ERROR_CLASS);
      if (!errorEl) {
    ...
    function validateField(field) {
      ...
      const customMessage = field.getAttribute(ERROR_ATTR);

      if (field.hasAttribute('required') && value === '') {
        setFieldInvalid(field, customMessage || 'This field is required.');
        return false;
      }
      ...
      if (type === 'email' && value !== '') {
        ...
        if (!emailPattern.test(value)) {
          setFieldInvalid(field, customMessage || 'Please enter a valid email address.');
          return false;
        }
      }
      ...
      if (type === 'tel' && value !== '') {
        ...
        if (!telPattern.test(value)) {
          setFieldInvalid(field, customMessage || 'Please enter a valid phone number.');
          return false;
        }
      }
      ...
      if (field.hasAttribute('minlength')) {
        ...
        if (value.length < min) {
          setFieldInvalid(field, customMessage || 'Minimum ' + min + ' characters required.');
          return false;
        }
      }
      ...
      if (field.hasAttribute('maxlength')) {
        ...
        if (value.length > max) {
          setFieldInvalid(field, customMessage || 'Maximum ' + max + ' characters allowed.');
          return false;
        }
      }
      ...
      if (field.hasAttribute('pattern') && value !== '') {
        ...
        if (!pattern.test(value)) {
          setFieldInvalid(field, customMessage || 'Please match the required format.');
          return false;
        }
      }
    ...
    contactForm.addEventListener('blur', function (e) {
      ...
      if (field.matches('input, textarea, select') && field.type !== 'submit' && field.type !== 'hidden') {
        validateField(field);
      }
    }, true);
  ...
  const ANIMATE_SELECTOR = '.fade-in, .animate-on-scroll, [data-animate]';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Bearflowers משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'ddf028f1-da1f-4042-88e4-30794357ff84',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'ddf028f1-da1f-4042-88e4-30794357ff84';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'ddf028f1-da1f-4042-88e4-30794357ff84';
